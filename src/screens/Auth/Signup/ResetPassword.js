...
export class ResetPassword extends React.Component {
  ...
  onSetPassword = () => {
    ...
    if (isPassword && isRepassword && password !== repassword) {
      ...
      this.props.dispatch(authAction.resetPassword(phoneNumber, password));
    ...
    return (
      <LinearGradient style={styles.container} colors={[MS.Blue500, '#0250c7']}>
        <Header noShadow style={styles.header}>
          ...
          <Right>
            <Button transparent disabled={!isContinue} onPress={() => this.onSetPassword()}>
              <Text style={[S.ft12B_W, !isContinue && { opacity: 0.5 }]}>{"Save password"}</Text>
            </Button>
          </Right>
        </Header>
